// Comet-like cursor trail, mimicking kitty's cursor_trail effect.
//
// Vendored from https://github.com/sahaj-b/ghostty-cursor-shaders (MIT).
// Tunables live in the CONFIGURATION block below; the active easing function
// is the single uncommented `float ease(float x)`.
//
// TRAIL_COLOR follows Ghostty's cursor color, so it stays in sync with the
// palette in ghostty/config without hardcoding a second copy of it.
